...
public class BookPackCommentAdapter extends BaseAdapter {
    ...
    public BookPackCommentAdapter(Fragment fromFragment, List<GroupBean> list, int source) {
        super();
...
//                holder.rl_item_comment_zan.setVisibility(View.VISIBLE);
...
//            holder.tv_item_comment_name.setCompoundDrawables(null, null, list.get(position).user_data.badge_Drawable(0.12f, 0.12f), null);
        ...
        //点赞数量
        if ("0".equals(list.get(position).digg_count)) {
            holder.tv_item_comment_zan.setText("");
        } else {
            holder.tv_item_comment_zan.setText(list.get(position).digg_count);
        }
        //评论数量
        if ("0".equals(list.get(position).comment_count)) {
            holder.tv_item_comment_pinglun.setText("");
        } else {
            holder.tv_item_comment_pinglun.setText(list.get(position).comment_count);
        }

//        if ("1".equals(list.get(position).comment_type)) {
//            if (list.get(position).reply_user_data != null) {
//                setText(holder.tv_item_comment_content, "回复",
//                        "@" + list.get(position).reply_user_data.name + ":",
//                        list.get(position).content, fromFragment.getActivity(),
//                        R.color.green);
//            } else {
//                setText(holder.tv_item_comment_content, "回复", "@:",
//                        list.get(position).content, fromFragment.getActivity(),
...
//            }
//        } else {
//            holder.tv_item_comment_content.setText(list.get(position).content);
//        }
    ...
    private static class ViewHolder {
        private RelativeLayout rl_item_comment_zan, rl_item_comment_pinglun;
        private ImageView iv_item_comment_avatar, iv_item_comment_sex;
        private TextView tv_item_comment_name, tv_item_comment_time,
                tv_item_comment_content, tv_item_comment_zan, tv_item_comment_pinglun;
    }
