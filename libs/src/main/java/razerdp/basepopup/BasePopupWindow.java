...
public abstract class BasePopupWindow implements BasePopup {
    ...
    private void tryToShowPopup(int res, View v) throws Exception {
        ...
        //传递了res
        if (res != 0 && v == null) {
            mPopupWindow.showAtLocation(((Activity)mContext).findViewById(res), Gravity.CENTER, 0, 0);
        }
        //什么都没传递，取顶级view的id
        if (res == 0 && v == null) {
            mPopupWindow.showAtLocation(((Activity)mContext).findViewById(android.R.id.content), Gravity.CENTER, 0, 0);
        }
